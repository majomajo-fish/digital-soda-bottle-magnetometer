...
このプログラムは、カメラを使用してレーザースポットの位置を連続的に記録するためのツールです。主に地磁気観測実験において、**デジタル画像記録方式ソーダボトル(ペットボトル)磁力計**のミラーに反射したレーザースポットの位置変化を測定するために使用されます。
実験の概要については、以下のリンクを参照してください：
- [説明動画](https://vimeo.com/1143342740?fl=pl&fe=sh)
- [講義資料](https://doi.org/10.14989/kds598559)

## 機能

- **リアルタイム画像取得**: カメラから連続的に画像を取得
- **レーザースポット位置解析**: 画像の赤チャンネルからスポット中心位置を計算
- **複数スポット対応**: カメラ1台あたり最大6つのスポットを同時に追跡可能
- **リアルタイム表示モード**: カメラ位置調整用のリアルタイムプレビュー機能
- **読み取る高さの自動調整**: スポットのy座標を自動追跡するモード
...
- パラメータファイルのc_y値を計測途中で `-1` に設定すると自動追尾モードになります
